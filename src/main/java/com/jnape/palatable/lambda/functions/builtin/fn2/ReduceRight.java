 ...
 */
public final class ReduceRight<A> implements Fn2<Fn2<? super A, ? super A, ? extends A>, Iterable<A>, Optional<A>> {

    private ReduceRight() {
    }

    @Override
    public final Optional<A> apply(Fn2<? super A, ? super A, ? extends A> fn, Iterable<A> as) {
        return reduceLeft(fn.flip(), reverse(as));
    }
    ...
    public static <A> Fn1<Iterable<A>, Optional<A>> reduceRight(Fn2<? super A, ? super A, ? extends A> fn) {
        return ReduceRight.<A>reduceRight().apply(fn);
    }

    public static <A> Optional<A> reduceRight(Fn2<? super A, ? super A, ? extends A> fn, Iterable<A> as) {

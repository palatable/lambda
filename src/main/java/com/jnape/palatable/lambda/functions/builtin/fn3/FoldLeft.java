 ...
 * value. If the <code>Iterable</code> is empty, just return the starting <code>B</code> value. Note that, as the name
 * implies, this function accumulates from left to right, such that <code>foldLeft(f, 0, asList(1, 2, 3, 4, 5))</code>
 * is evaluated as <code>f(f(f(f(f(0, 1), 2), 3), 4), 5)</code>.
 * <p>
...
public final class FoldLeft<A, B> implements Fn3<Fn2<? super B, ? super A, ? extends B>, B, Iterable<A>, B> {
    ...
    @Override
    public B apply(Fn2<? super B, ? super A, ? extends B> fn, B acc, Iterable<A> as) {
        B accumulation = acc;
        for (A a : as)

...
/**
 * Given an <code>Iterable</code> of <code>A</code>s, a starting value <code>B</code>, and a <code>{@link Fn2}&lt;B, A,
 * B&gt;</code>, iteratively accumulate over the <code>Iterable</code>, collecting each function application result,
 * finally returning an <code>Iterable</code> of all the results. Note that, as the name implies, this function
 * accumulates from left to right, such that <code>scanLeft(f, 0, asList(1,2,3,4,5))</code> is evaluated as 0, f(0, 1),
 * f(f(0, 1), 2), f(f(f(0, 1), 2), 3), f(f(f(f(0, 1), 2), 3), 4), f(f(f(f(f(0, 1), 2), 3), 4), 5).
 *
...
public final class ScanLeft<A, B> implements Fn3<Fn2<? super B, ? super A, ? extends B>, B, Iterable<A>, Iterable<B>> {
    ...
    @Override
    public Iterable<B> apply(Fn2<? super B, ? super A, ? extends B> fn, B b, Iterable<A> as) {

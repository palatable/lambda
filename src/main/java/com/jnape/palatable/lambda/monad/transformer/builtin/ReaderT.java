...
import com.jnape.palatable.lambda.functor.builtin.Reader;
...
/**
 * A {@link MonadT monad transformer} for any {@link Fn1 function} from some
 * type <code>R</code> to some {@link MonadRec monadic} embedding
 * <code>{@link MonadRec}&lt;A, M&gt;</code>.
 *
 * @param <R> the input type
 * @param <M> the returned {@link MonadRec}
 * @param <A> the embedded output type
 */
public final class ReaderT<R, M extends MonadRec<?, M>, A> implements MonadReader<R, A, ReaderT<R, M, ?>>,
		Cartesian<R, A, ReaderT<?, M, ?>>, MonadT<M, A, ReaderT<R, M, ?>, ReaderT<R, ?, ?>> {
	...
	/**
	 * Map the current {@link Monad monadic} embedding to a new one in a potentially
	 * different {@link Monad}.
	 *
	...
	/**
	 * Left-to-right composition between {@link ReaderT} instances running under the
	 * same effect and compatible between their inputs and outputs.
	 *
	...
	public <B> ReaderT<R, M, B> trampolineM(
		...
		return readerT(r -> runReaderT(r)
				.trampolineM(a -> fn.apply(a).<ReaderT<R, M, RecursiveResult<A, B>>>coerce().runReaderT(r)));
	}
	 ...
	 * @return the mapped {@link Reader}
	 */
	public <B> ReaderT<R, M, B> rmap(Fn2<? super R, ? super A, ? extends B> fn) {
		return readerT(r -> runReaderT(r).fmap(fn.apply(r)));
	}
	 ...
	 * @return the mapped {@link Reader}
	 */
	public <B> ReaderT<R, M, B> rmap(Fn1<Tuple2<? super A, ? super R>, ? extends B> fn) {
		return readerT(r -> runReaderT(r).fmap(a -> fn.apply(tuple(r, a))));
	}
	...
	/**
	 * Lift a {@link Fn1 function} (<code>R -&gt; {@link Monad}&lt;A, M&gt;</code>)
